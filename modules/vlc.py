...
class VlcCog(commands.Cog):
    ...
    async def display_media_vlc(self, ctx: commands.Context, link):
        ...
        with yt_dlp.YoutubeDL({}) as ytl:
            ...
            await self.insert_request(id, link, ctx.message.author.display_name, info.get('fulltitle'),
                                      info.get('creator'),
    ...
    async def insert_request(self, id, url, requestor, title, creator, likes, dislikes, status="NOT PLAYING",
                             approved='F', queue_position=None):
    ...
    async def play_next(self, ctx: commands.Context, queue_pos=None):
        ...
        await self.obs._setSourceSettings("vlc_link",
                                          {"playlist": [{"value": playurl, 'hidden': False, 'selected': False}]})
        ...
        ergonomic = "Looks uncomfortable; supposedly better for you???"  # Remove this it isn't being used.
    ...
    async def whats_playing(self, ctx: commands.Context):
    ...
    async def get_queue(self, ctx: commands.Context):  # Don't use Camel Case in Python
        retrieve_queue = """
